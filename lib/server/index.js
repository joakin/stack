...
import {newRoom, addParticipant, removeParticipant} from '../client/state'
...
server.get('/api/room/:name', (req, res) => {
  ...
  app.rooms[name] = app.rooms[name] || newRoom({name})
...
server.post('/api/add/:room_name', (req, res) => {
  ...
  const room = app.rooms[room_name] || newRoom({name: room_name})
